/**
 * ONES4 Print Designer Server - Fixed Version
 * This replaces the problematic server.ts file
 */

import express from 'express'
import payload from 'payload'
import { resolve } from 'path'
import { printDesignerConfig } from './print-designer-config'
...
const start = async () => {
  ...
  await payload.init({
    ...
    onInit: async () => {
      ...
      payload.logger.info(`🔗 Connected to ONES4 Store: ${printDesignerConfig.store.publicDomain}`)
    },
  })
  ...
  app.get('/api/print-designer/config', (req, res) => {
    // Only send safe config data to frontend
    const safeConfig = {
      store: printDesignerConfig.store,
      customization: printDesignerConfig.customization,
      neonEngine: printDesignerConfig.neonEngine,
      output: printDesignerConfig.output,
      features: printDesignerConfig.features
    }
    res.json(safeConfig)
  })
  ...
  console.log(`📊 Admin: http://localhost:${process.env.PORT || 8080}/admin`)
  console.log(`🔗 Store: ${printDesignerConfig.store.publicDomain}`)
  console.log(`🎨 App: ${printDesignerConfig.appName} v${printDesignerConfig.version}`)
}
...
async function getCustomizableProducts() {
  // This would fetch from Shopify API using the config
  const storeUrl = `https://${printDesignerConfig.store.domain}`
  ...
  return [
    {
      ...
      vendor: printDesignerConfig.store.name
    },
    {
      id: '987654321',
      title: 'Neon LED Sign Custom',
      type: 'Neon Signs', 
      tags: ['neon', 'customizable', 'led', 'sign'],
      vendor: printDesignerConfig.store.name
    }
  ]
}
