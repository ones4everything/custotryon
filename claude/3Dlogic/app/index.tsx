...
type DesignMethod = 'vinyl' | 'sublimation';
type PanelMode = 'basic' | 'advanced';
type ProductCategory = 'shirts' | 'hoodies' | 'shorts' | 'accessories';
...
const DESIGN_DRAFT_KEY = 'o4-design-draft-v1';
const DESIGN_STORE_KEY = 'o4-design-store-v1';
...
const ACCESSORY_PRODUCTS: ProductType[] = ['sunglasses', 'tote', 'fannypack'];

const CATEGORY_TABS: { id: ProductCategory; label: string; disabled?: boolean }[] = [
  { id: 'shirts', label: 'Shirts' },
  { id: 'hoodies', label: 'Hoodies' },
  { id: 'shorts', label: 'Shorts', disabled: true },
  { id: 'accessories', label: 'Accessories' }
];

const SUBLIMATION_SUPPORTED: Record<ProductType, boolean> = {
  tshirt: true,
  hoodie: true,
  sunglasses: false,
  tote: true,
...
const PRODUCT_ALIASES: Record<string, ProductType> = {
  ...
  'fanny_pack': 'fannypack'
};

const getCategoryForProduct = (value: ProductType): ProductCategory => {
  if (value === 'tshirt') return 'shirts';
  if (value === 'hoodie') return 'hoodies';
  if (ACCESSORY_PRODUCTS.includes(value)) return 'accessories';
  return 'shirts';
};

const resolveProductType = (value: string | null): ProductType | null => {
  if (!value) return null;
  const key = value.toLowerCase().trim();
  return PRODUCT_ALIASES[key] || null;
};
...
const loadDraft = (): DesignPayload | null => {
  try {
    return safeJsonParse<DesignPayload>(localStorage.getItem(DESIGN_DRAFT_KEY));
  } catch (err) {
    return null;
  }
};

const saveDraft = (payload: DesignPayload) => {
  try {
    localStorage.setItem(DESIGN_DRAFT_KEY, JSON.stringify(payload));
  } catch (err) {
  }
};

const saveDesignLocal = (payload: DesignPayload): { id: string } => {
  const id = payload.id || crypto.randomUUID();
  let store: Record<string, DesignPayload> = {};
  try {
    store = safeJsonParse<Record<string, DesignPayload>>(localStorage.getItem(DESIGN_STORE_KEY)) || {};
  } catch (err) {
    store = {};
  }
  store[id] = { ...payload, id };
  try {
    localStorage.setItem(DESIGN_STORE_KEY, JSON.stringify(store));
  } catch (err) {
  }
  return { id };
};
    ...
    <group ref={meshRef}>
      <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
        ...
        <Html 
          ...
          scale={0.25} // Increased resolution by doubling dimensions and halving scale (0.5 -> 0.25)
          style={{
             width: '1024px', // Doubled from 512px
             height: '1024px', // Doubled from 512px
        ...
        >
          <div className="w-full h-full relative">
             ...
             <div className="absolute inset-0 opacity-90 p-16 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
...
const App = () => {
  ...
  const [designMethod, setDesignMethod] = useState<DesignMethod>('vinyl');
  const [lastAccessory, setLastAccessory] = useState<ProductType>('sunglasses');
  ...
  const productCategory = getCategoryForProduct(product);
  const supportsSublimation = SUBLIMATION_SUPPORTED[product];
  const saveStatusTone = cartState === 'added'
    ? 'saved'
    : cartState === 'error'
      ? 'error'
      : saveState === 'saving'
  ...
  const saveStatusLabel = cartState === 'added'
    ? 'Added to cart'
    : cartState === 'error'
  ...
  const handleCategorySelect = (category: ProductCategory) => {
    ...
    if (category === 'accessories') {
      setProduct(lastAccessory || 'sunglasses');
    }
  };
  ...
  useEffect(() => {
    ...
    const productParam = resolveProductType(params.get('product'));
    const variantParam = params.get('variant') || params.get('variant_id') || params.get('variantId');
    const nameParam = params.get('design') || params.get('title') || params.get('name');
    const returnParamRaw = params.get('return') || params.get('back');
    const returnParam = returnParamRaw ? decodeURIComponent(returnParamRaw) : null;
    const ignoreDraft = params.get('draft') === '0' || params.get('new') === '1';

    if (variantParam) setVariantId(variantParam);
    if (returnParam) setReturnUrl(returnParam);

    if (!ignoreDraft) {
      const draft = loadDraft();
      if (draft && draft.elements && draft.elements.length) {
        setElements(draft.elements);
        if (!nameParam) setDesignName(draft.name || DEFAULT_DESIGN_NAME);
        if (!productParam && draft.product) setProduct(draft.product);
        if (draft.id) setDesignId(draft.id);
        if (draft.designMethod) setDesignMethod(draft.designMethod);
      }
    }

    if (productParam) setProduct(productParam);
    if (nameParam) setDesignName(nameParam);

    setIsHydrated(true);
  }, []);

  useEffect(() => {
    if (ACCESSORY_PRODUCTS.includes(product)) {
      setLastAccessory(product);
    }
  }, [product]);

  useEffect(() => {
    if (!supportsSublimation && designMethod !== 'vinyl') {
      setDesignMethod('vinyl');
    }
  }, [supportsSublimation, designMethod]);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      const payload: DesignPayload = {
        ...
        name: designName.trim() || DEFAULT_DESIGN_NAME,
        product,
        designMethod,
        elements,
  ...
  }, [designName, product, designMethod, elements, designId, isHydrated]);

  const persistDesign = async (): Promise<{ id: string }> => {
    const payload: DesignPayload = {
      id: designId || undefined,
      name: designName.trim() || DEFAULT_DESIGN_NAME,
      product,
    ...
    const endpoint = env.VITE_DESIGN_SAVE_URL;
    if (endpoint) {
      ...
      const id = String(data?.id || data?._id || payload.id || crypto.randomUUID());
      setDesignId(id);
      saveDraft({ ...payload, id });
      return { id };
    }

    const local = saveDesignLocal(payload);
    setDesignId(local.id);
    saveDraft({ ...payload, id: local.id });
    return local;
  };
  ...
  const handleAddToCart = async () => {
    ...
    if (!variantId) {
      setCartState('error');
      setCartError('Missing variant id');
      return;
    }
    ...
    try {
      ...
      const payloadVariant = Number.isFinite(numericVariant) ? numericVariant : variantId;
      const res = await fetch('/cart/add.js', {
        ...
        body: JSON.stringify({
          ...
          properties: {
            designId: saved.id,
            designName: designName.trim() || DEFAULT_DESIGN_NAME
          }
        })
      });
      if (!res.ok) {
  ...
  useEffect(() => {
    const handleGlobalMove = (e: MouseEvent) => {
      ...
      if(rotating.active && rotating.startEl) {
         ...
         // This actually helps keeps the slider centered
    ...
    if (el.type === 'text') {
      switch (el.texture) {
        ...
        case 'puff':
          ...
          // Removed rotation from here so the container controls it
          baseStyle.transform = 'scale(1.05)'; // Slight pop
          break;
        case 'vinyl':
        ...
        <div className="neo-shell">
          <header className="neo-header">
            <div className="neo-breadcrumb">
              {returnUrl ? (
                <button className="neo-back-btn" onClick={handleReturn} title="Back to product">
                  <span className="text-[#00aeff]">Back</span>
                </button>
              ) : (
                <>
            ...
            <div className="neo-center">
              <input
                className="neo-input neo-title-input"
                value={designName}
                onChange={(e) => setDesignName(e.target.value)}
                placeholder="Design name"
                aria-label="Design name"
              />
              <div className={`neo-status neo-status--${saveStatusTone}`}>
                {saveStatusLabel}
              </div>
            </div>
            ...
            <div className="neo-actions">
              ...
              >
                {saveState === 'saving' ? <Loader2 size={18} className="animate-spin" /> : <Upload size={18} />}
              </button>
              <button
                className="neo-icon-btn"
                onClick={handleAddToCart}
                title={variantId ? 'Add to cart' : 'Missing variant id'}
                disabled={!variantId || cartState === 'loading'}
              >
                {cartState === 'loading' ? (
              ...
              <button
                className="neo-icon-btn"
                onClick={(e) => { e.stopPropagation(); setIsPreview(true); setSelectedId(null); }}
                title="Preview mode"
              >
                <Eye size={18} />
              </button>
                ...
                {CATEGORY_TABS.map((tab) => {
                  ...
                  return (
                    <button
                      key={tab.id}
                      type="button"
                      className={`canvas-tab ${isActive ? 'active' : ''} ${tab.disabled ? 'disabled' : ''}`}
                      onClick={() => {
                        ...
                        {elements.map(el => {
                           ...
                           <div
                              ...
                              style={{
                                 ...
                                 width: el.type === 'text' ? 'auto' : el.width,
                                 height: el.type === 'text' ? 'auto' : el.height,
                                 transform: `rotate(${el.rotation}deg)`, // Always apply rotation here
                                 opacity: el.opacity,
                           ...
                           >
                              ...
                              {isSelected && (
                                 <>
                                    {/* Rotation Handle */}
                                    <div 
                                       className="absolute -top-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 cursor-grab active:cursor-grabbing handle-control z-50"
                                       onMouseDown={(e) => handleRotateStart(e, el)}
                                    >
                                       {rotating.active && rotating.startEl?.id === el.id && (
                                          <div className="absolute -top-8 px-2 py-1 bg-[#00aeff] text-black text-[10px] font-bold rounded whitespace-nowrap">
                                             {Math.round(el.rotation)}°
                                          </div>
                                       )}
                                       <div className="w-6 h-6 rounded-full bg-white text-black flex items-center justify-center shadow-[0_0_10px_#00aeff] border border-[#00aeff] hover:scale-110 transition-transform">
                                          <RotateCw size={14} />
                                       </div>
                                    ...
                                    {/* Corners */}
                                    <div onMouseDown={(e) => handleResizeStart(e, 'nw', el)} className="absolute -top-1.5 -left-1.5 w-3 h-3 bg-white border border-[#00aeff] cursor-nw-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 'ne', el)} className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-white border border-[#00aeff] cursor-ne-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 'sw', el)} className="absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-white border border-[#00aeff] cursor-sw-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 'se', el)} className="absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-white border border-[#00aeff] cursor-se-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    {/* Edges */}
                                    <div onMouseDown={(e) => handleResizeStart(e, 'n', el)} className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-4 h-1.5 bg-white border border-[#00aeff] cursor-ns-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 's', el)} className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-4 h-1.5 bg-white border border-[#00aeff] cursor-ns-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 'w', el)} className="absolute top-1/2 -translate-y-1/2 -left-1.5 w-1.5 h-4 bg-white border border-[#00aeff] cursor-ew-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                    <div onMouseDown={(e) => handleResizeStart(e, 'e', el)} className="absolute top-1/2 -translate-y-1/2 -right-1.5 w-1.5 h-4 bg-white border border-[#00aeff] cursor-ew-resize z-50 rounded-full handle-control hover:scale-125 transition-transform"></div>
                                 </>
                              )}
                           </div>
                           );
          ...
          {!isPreview && (
            <div className="options-panel animate-fade-in-right">
              ...
              <div className="option-block border-l-2 border-l-[#ff62e5]">
                 ...
                 <div className="flex gap-2 mt-2">
                    ...
                    <button 
                        ...
                        className="btn-primary flex-1 flex items-center justify-center gap-2 text-[10px] !bg-[#ff62e5]/10 !text-[#ff62e5] !border-[#ff62e5]"
                    >
                        <Camera size={16} /> TRY-ON
                    </button>
                 </div>
              </div>
              ...
              <div className="panel-tabs" role="tablist" aria-label="Panel mode">
                <button
                  type="button"
                  className={`panel-tab ${panelMode === 'basic' ? 'active' : ''}`}
                  onClick={() => setPanelMode('basic')}
                  aria-pressed={panelMode === 'basic'}
                >
                  Basic
                </button>
                <button
                  type="button"
                  className={`panel-tab ${panelMode === 'advanced' ? 'active' : ''}`}
                  onClick={() => setPanelMode('advanced')}
                  aria-pressed={panelMode === 'advanced'}
                >
                  Advanced
                </button>
              </div>
              ...
              {panelMode === 'basic' && (
                <>
                  ...
                  <div className="option-block">
                    <div className="option-title flex items-center gap-2"><Shirt size={12}/> Change Product</div>
                    <div className="product-grid">
                      <button
                        type="button"
                        className={`product-card ${productCategory === 'shirts' ? 'active' : ''}`}
                        onClick={() => handleCategorySelect('shirts')}
                        aria-pressed={productCategory === 'shirts'}
                      >
                        <span className="product-card-icon"><Shirt size={18} /></span>
                        <span className="product-card-label">Shirts</span>
                      </button>
                      <button
                        type="button"
                        className={`product-card ${productCategory === 'hoodies' ? 'active' : ''}`}
                        onClick={() => handleCategorySelect('hoodies')}
                        aria-pressed={productCategory === 'hoodies'}
                      >
                        <span className="product-card-icon"><Layers size={18} /></span>
                        <span className="product-card-label">Hoodies</span>
                      </button>
                      <button
                        type="button"
                        className="product-card disabled"
                        onClick={() => handleCategorySelect('shorts')}
                        aria-disabled="true"
                        disabled
                      >
                        <span className="product-card-icon"><Box size={18} /></span>
                        <span className="product-card-label">Shorts</span>
                      </button>
                      <button
                        type="button"
                        className={`product-card ${productCategory === 'accessories' ? 'active' : ''}`}
                        onClick={() => handleCategorySelect('accessories')}
                        aria-pressed={productCategory === 'accessories'}
                      >
                        <span className="product-card-icon"><Briefcase size={18} /></span>
                        <span className="product-card-label">Accessories</span>
                      </button>
                    </div>
                    ...
                    <div className="accessory-grid">
                      <button
                        type="button"
                        className={`accessory-card ${product === 'sunglasses' ? 'active' : ''}`}
                        onClick={() => setProduct('sunglasses')}
                        aria-pressed={product === 'sunglasses'}
                      >
                        <Glasses size={16} />
                        Sunglasses
                      </button>
                      <button
                        type="button"
                        className={`accessory-card ${product === 'tote' ? 'active' : ''}`}
                        onClick={() => setProduct('tote')}
                        aria-pressed={product === 'tote'}
                      >
                        <ShoppingBag size={16} />
                        Tote
                      </button>
                      <button
                        type="button"
                        className={`accessory-card ${product === 'fannypack' ? 'active' : ''}`}
                        onClick={() => setProduct('fannypack')}
                        aria-pressed={product === 'fannypack'}
                      >
                        <Briefcase size={16} />
                        Fanny Pack
                      </button>
              ...
              <div className="option-block">
                <div className="option-title flex items-center gap-2"><Droplets size={12}/> Design Method</div>
                <div className="method-toggle">
                  <button
                    type="button"
                    className={`method-option ${designMethod === 'vinyl' ? 'active' : ''}`}
                    onClick={() => setDesignMethod('vinyl')}
                    aria-pressed={designMethod === 'vinyl'}
                  >
                    Vinyl
                  </button>
                  <button
                    type="button"
                    className={`method-option ${designMethod === 'sublimation' ? 'active' : ''} ${supportsSublimation ? '' : 'disabled'}`}
                    onClick={() => supportsSublimation && setDesignMethod('sublimation')}
                    disabled={!supportsSublimation}
                    aria-pressed={designMethod === 'sublimation'}
                    aria-disabled={!supportsSublimation}
                  >
                    Sublimation
                  </button>
                </div>
                {!supportsSublimation && (
                  <div className="method-note">Sublimation is available only on supported fabrics.</div>
                )}
              </div>
                    ...
                    <>
                      ...
                      {selectedElement.type === 'text' ? (
                        <div className="space-y-3">
                          <div>
                            <div className="text-[10px] text-gray-500 uppercase mb-2">Content</div>
                            <input
                              type="text"
                              className="neo-input"
                          ...
                          <div>
                            <div className="text-[10px] text-gray-500 uppercase mb-2">Font</div>
                            <select
                              className="neo-input"
                              value={selectedElement.fontFamily || FONT_OPTIONS[0].name}
                          ...
                          <div>
                            <div className="flex justify-between text-[10px] text-gray-500 uppercase mb-1">
                              <span>Size</span>
                              <span className="text-[#00aeff]">{selectedElement.fontSize || 24}px</span>
                            </div>
                            <input
                        ...
                        <div className="panel-note">
                          Basic mode focuses on text edits. Switch to Advanced for full controls.
                        </div>
                      )}
                    </>
                  ) : (
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="border-t border-white/10 pt-4">
                             ...
                             <div className="mb-3">
                                <div className="flex justify-between items-center text-[9px] text-gray-400 mb-1">
                                   <span className="flex items-center gap-1"><Palette size={10}/> OUTLINE</span>
                          ...
                          <div>
                            ...
                            <input
                               ...
                               className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-[#00aeff] mb-2"
                               value={selectedElement.rotation}
                               onChange={(e) => updateElement(selectedElement.id, { rotation: parseInt(e.target.value) })}
                            />
                            <div className="flex gap-2">
                               <button onClick={() => updateElement(selectedElement.id, { rotation: -90 })} className="flex-1 py-1 bg-white/5 rounded text-[10px] hover:bg-white/10 flex items-center justify-center gap-1 border border-transparent hover:border-[#00aeff]/30"><RotateLeft size={10}/> -90??</button>
                               <button onClick={() => updateElement(selectedElement.id, { rotation: 0 })} className="flex-1 py-1 bg-white/5 rounded text-[10px] hover:bg-white/10 flex items-center justify-center gap-1 border border-transparent hover:border-[#00aeff]/30">0??</button>
                               <button onClick={() => updateElement(selectedElement.id, { rotation: 90 })} className="flex-1 py-1 bg-white/5 rounded text-[10px] hover:bg-white/10 flex items-center justify-center gap-1 border border-transparent hover:border-[#00aeff]/30"><RotateRight size={10}/> +90??</button>
                            </div>
                          </div>
